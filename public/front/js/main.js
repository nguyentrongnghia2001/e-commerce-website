     ...
     $.ajax({
        ...
        success: function(response){
           ...
           cart_tbody.children().remove();
            ...
            $('.subtotal span').text('0đ');
            $('.cart-total span').text('0đ');
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(qty === 0){
                ...
                cartHover_existItem.find('.product-selected p').text((response['cart'].price + ' x ' + response['cart'].qty) + 'đ');
            }

           //Xử lý trong trang shop/cart 
           var cart_tbody = $('.cart-table table tbody');
           var cart_existItem = cart_tbody.find("tr" + "[data-rowid='" + rowId + "']");

           if(qty === 0){
                cart_existItem.remove();
            } else{
                cart_existItem.find('.total-price').text((response['cart'].price * response['cart'].qty) + 'đ');
            }
